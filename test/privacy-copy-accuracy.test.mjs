...
test('teen Pages screen describes account sync without a sole-reader promise', () => {
  const screen = read('screens/PagesScreen.tsx');

  // Journal entries upsert to Supabase; operators hold service-role access.
  assert.match(screen, /sync to your account/i);
  assert.match(screen, /Parent Pages/i);

  // This absolute conflicts with the implemented storage path.
  assert.doesNotMatch(screen, /only you can see/i);
});
