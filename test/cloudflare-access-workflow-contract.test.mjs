...
test('Cloudflare Access audit is exact-head, credential-minimal, and action-pinned', () => {
  ...
  assert.ok(!workflow.includes('secrets.CLOUDFLARE_ACCESS_WRITE_API_TOKEN'), 'read-only Access audit must never receive Production Access write authority');
  assert.ok(!workflow.includes('secrets.CLOUDFLARE_ACCESS_API_TOKEN'), 'read-only Access audit must not depend on the legacy ambiguous Access secret name');
