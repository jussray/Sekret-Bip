...
When the deployment supports them, publish `/robots.txt`, `/sitemap.xml`, `/llms.txt`, and `/crawlers.json` with public-safe truth only. These are discovery/policy surfaces, not authentication controls.
