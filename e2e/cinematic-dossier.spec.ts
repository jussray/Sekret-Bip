...
async function expectRenderedImagesDecoded(targets: Locator, label: string) {
  await expect.poll(async () => targets.evaluateAll(async nodes => {
    const images = nodes
    ...
    await Promise.all(images.map(async image => {
      ...
      try {
        ...
        // The diagnostic pass below reports the exact failed source and dimensions.
      }
    }));
    ...
    for (const node of nodes) {
      ...
      if (!image) {
        failures.push({ src: null, complete: null, naturalWidth: null, naturalHeight: null, reason: 'no rendered img descendant' });
        continue;
      }
    ...
    return failures;
  }), { message: `Wait for ${label} to finish decoding`, timeout: 10_000 }).toEqual([]);
}
...
async function captureCompleteBoardEvidence(page: Page, board: Locator, viewport: (typeof VIEWPORTS)[number], testInfo: TestInfo) {
  ...
  try {
    ...
    expect(boardBox!.height, 'Complete dossier board must extend beyond the phone viewport').toBeGreaterThan(viewport.name === 'mobile' ? viewport.height : 900);
...
async function captureSorianGateEvidence(page: Page, viewport: (typeof VIEWPORTS)[number], testInfo: TestInfo) {
  const gate = page.getByTestId('sorian-character-recognition-gate');
