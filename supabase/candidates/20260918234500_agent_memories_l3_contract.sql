-- Se'kret Bip — candidate-only L3 durable companion memory contract.
--
-- NOT A PRODUCTION MIGRATION. The Supabase GitHub integration auto-runs files
-- under supabase/migrations on merge, so this reviewed SQL stays outside that
-- directory until the production activation gate is explicitly satisfied.
