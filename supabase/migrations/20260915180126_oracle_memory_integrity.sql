-- Restore the Oracle persistence contract that the current services already use.
-- This migration is additive/idempotent and does not expose session contents to
-- parents or anonymous sessions.
...
create index if not exists oracle_session_log_user_mode_idx
  on public.oracle_session_log (user_id, mode, completed_at desc);
...
create policy "oracle_session_log: owner delete"
  on public.oracle_session_log for delete
  to authenticated
  using (public.is_non_anonymous_user() and auth.uid() = user_id);

-- Production drift had restored these policies to role PUBLIC and removed the
-- non-anonymous predicate even though the canonical hardening migrations exist.
-- Reassert the intended contract before adding more sensitive continuity data.
alter policy "Enable users to view their own data only"
  on public.oracle_records
  to authenticated
  using (public.is_non_anonymous_user() and auth.uid() = user_id);

alter policy "oracle_records: owner all"
  on public.oracle_records
  to authenticated
  using (public.is_non_anonymous_user() and auth.uid() = user_id)
  with check (public.is_non_anonymous_user() and auth.uid() = user_id);

alter policy "oracle_sessions: owner read"
  on public.oracle_sessions
  to authenticated
  using (public.is_non_anonymous_user() and auth.uid() = user_id);

alter policy "oracle_sessions: owner insert"
  on public.oracle_sessions
  to authenticated
  with check (public.is_non_anonymous_user() and auth.uid() = user_id);

alter policy "oracle_sessions: owner update"
  on public.oracle_sessions
  to authenticated
  using (public.is_non_anonymous_user() and auth.uid() = user_id)
  with check (public.is_non_anonymous_user() and auth.uid() = user_id);

alter policy "oracle_sessions: owner delete"
  on public.oracle_sessions
  to authenticated
  using (public.is_non_anonymous_user() and auth.uid() = user_id);
