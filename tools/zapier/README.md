...
This package keeps Zapier SDK access on a server-side developer boundary. It is deliberately isolated from the Expo and React Native application bundle.
...
`npm run start` performs a read-only authentication check with `getProfile()`. It does not run an app action or access Se’kret Bip user data.
...
```bash
npx zapier-sdk create-client-credentials "sekret-bip-server-tooling"
```
...
Never expose Zapier credentials through Expo public variables, client code, screenshots, logs, issues, test fixtures, or app telemetry. Credential creation, rotation, deployment, and live write actions remain separate approval gates.
